...
const regexes = {
  ...
  device: [[
    ...
    // Lenovo
    /(ideatab[-\w ]+)/i,
    /lenovo ?(s[56]000[-\w]+|tab[\w ]+|yt[-\w]{6}|tb[-\w]{6})/i,
    ...
    /((?:sd|kf)[0349hijor-uw]+)( bui|\)).+silk\//i, // Fire Phone
    ...
    /\b(xtreme_)?(v(1[045]|2[015]|[3469]0|7[05])) b/i, // Voice Xtreme Phones
...
type ParserSection = 'browser' | 'cpu' | 'device' | 'engine' | 'os'

export type ParserExtensions = Partial<Record<ParserSection, unknown[]>>
...
interface ParserContext {
  ua: string
  isSelfNavigator: boolean
  navigator: UANavigator | undefined
  userAgentData: UANavigator['userAgentData']
...
function parseBrowserFromContext(context: ParserContext): IBrowser {
...
function parseCPUFromContext(context: ParserContext): ICPU {
  const cpu: ICPU = {
...
function parseDeviceFromContext(context: ParserContext): IDevice {
...
function parseEngineFromContext(context: ParserContext): IEngine {
...
function parseOSFromContext(context: ParserContext): IOS {
  const os: IOS = {
    name: undefined,
    version: undefined,
  }
...
export function parseBrowser(uastringOrExtensions?: ParserInput, extensions?: ParserExtensions): IBrowser {
  return parseBrowserFromContext(createParserContext(uastringOrExtensions, extensions))
}

export function parseCPU(uastringOrExtensions?: ParserInput, extensions?: ParserExtensions): ICPU {
  return parseCPUFromContext(createParserContext(uastringOrExtensions, extensions))
}

export function parseDevice(uastringOrExtensions?: ParserInput, extensions?: ParserExtensions): IDevice {
  return parseDeviceFromContext(createParserContext(uastringOrExtensions, extensions))
}

export function parseEngine(uastringOrExtensions?: ParserInput, extensions?: ParserExtensions): IEngine {
  return parseEngineFromContext(createParserContext(uastringOrExtensions, extensions))
}

export function parseOS(uastringOrExtensions?: ParserInput, extensions?: ParserExtensions): IOS {
  return parseOSFromContext(createParserContext(uastringOrExtensions, extensions))
}
